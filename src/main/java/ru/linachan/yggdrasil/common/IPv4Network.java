...
public class IPv4Network {
    ...
    public boolean inRange(Inet4Address inetAddress) {
        return inRange(inetAddress.getHostAddress());
    }

    public boolean inRange(String inetAddress) {
        long inetAddressNumber = ip2long(inetAddress);
        return (networkAddress <= inetAddressNumber)&&(inetAddressNumber <= networkAddress + Math.round(Math.pow(2, 32 - netMaskBits)) - 1);
    }

    public Inet4Address getNetworkAddress() throws UnknownHostException {
        return (Inet4Address) InetAddress.getByName(long2ip(networkAddress));
    }
